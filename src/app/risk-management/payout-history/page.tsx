...
function PayoutHistoryView() {
  ...
  async function lookup(account: string) {
    ...
    setQ("");
    try {
      const res = await fetch(`/api/payout-history?account=${encodeURIComponent(account)}`);
  ...
  function exportExcel() {
    ...
    XLSX.writeFile(wb, `PayoutHistory${searchedAccount}.xlsx`);
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 mb-1">Payout History</h1>
        <p className="text-slate-600">
          Look up every Anypay merchant that has paid out to a recipient&apos;s bank account
          (successful payouts only), aggregated by merchant.
        </p>
      </div>
                  ...
                  {shown === 0 && (
                    <tr>
                      <td colSpan={12} className="text-center py-12 text-slate-500">
                        {total === 0
                          ? `No successful payouts found to account ${searchedAccount}.`
